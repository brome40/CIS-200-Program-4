...
public abstract class Parcel : IComparable<Parcel>
{
    ...
    //Compare Method
    //Precondition:  None
    //Postcondition: Default ordering is created (ascending by cost)
    public int CompareTo(Parcel p)
    {
        //if second object is null, it will always be less than any other object
        if (p == null)
            return 1;
        //if not, compare based on cost
